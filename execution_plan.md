...
| # | Proposal in `improvement2.txt` | Priority in Text | Codebase Reality & Assessment | Verdict & Execution Strategy |
|---|---|---|---|---|
| **1** | **Replace startup schema migration with Alembic** | 🔴 Very high | Current auto-migration in `migrate_database()` dynamically adds columns on boot via `ALTER TABLE ADD COLUMN`. Alembic is documented in `ARCHITECTURE.md` as planned for v2 when table drops/renames occur. Self-hosted single-container deployments rely on zero-maintenance, automated startup upgrades without mandatory CLI intervention. | **Deferred to v2 Roadmap**: Retain automatic startup migration for v1; maintain documented Alembic adoption trigger in `ARCHITECTURE.md`. |
| **2** | **Improve `MediaItem` data model (Split into typed metadata tables)** | 🟠 High | `MediaItem` uses SQLAlchemy mixins (`MediaItemMixin`, `PlayableItemMixin`, etc.) with sparse nullable columns. SQLite stores `NULL` with 0–1 byte overhead. Splitting into 5+ separate tables adds 5+ JOINs to every query, increases query latency, and breaks existing user databases. | **Deferred to v2 Roadmap**: Retain single-table mixin architecture for SQLite speed and simplicity; keep as architectural evolution path. |
| **3** | **Make file identity stronger than SHA-256 alone (Fast fingerprinting)** | 🔴 Very high | `indexer.py` *already* skips hashing when `file_size` and `file_mtime` are unchanged. However, new/modified large files (e.g., 20–50 GB videos) compute full SHA-256 synchronously, which is expensive on NAS/Pi hardware. | **✅ Applicable & High Priority (P0)**: Implement two-stage identity: fast fingerprint (size + first 64KB + last 64KB hash) for large files (> 32 MB), plus increased 1 MB I/O read buffer. |
| **4** | **Turn scanning into incremental indexing engine** | 🔴 Very high | **Already Implemented!** `DebouncedLibraryChangeHandler` in `src/aarkib/services/watcher.py` already debounces filesystem events per file, indexes single files incrementally, and immediately syncs FTS on create/modify/move/delete. | **✅ Refinement**: Optimize timer cleanup to prevent thread explosion during massive bulk directory copies. |
| **5** | **Add a proper search engine (FTS5 with field queries)** | 🔴 Very high | **Core FTS5 Already Implemented!** `src/aarkib/services/search.py` uses SQLite FTS5, porter unicode61, BM25 ranking, and faceted grouping. **Missing**: Query parser strips colons, so field qualifiers (`author:`, `series:`, `tag:`, `type:`) are not yet converted to FTS column filters. | **✅ Applicable & High Priority (P0)**: Enhance `parse_fts_query()` to support field qualifiers (`author:`, `creator:`, `series:`, `tag:`, `type:`, `title:`) mapped directly to FTS5 column queries. |
| **6** | **Metadata enrichment intelligence (Provenance & locks)** | 🟠 High | **Already Largely Implemented!** `is_field_locked` protects user edits across all fields; `MetadataSearchResult.score` calculates confidence (threshold >= 0.5). | **✅ Refinement**: Ensure field lock state is visually transparent and never overridden by automated scans. |
| **7** | **Add proper backup/restore** | 🔴 Very high | **Completely Missing!** No native backup or disaster recovery mechanism currently exists in Aarkib. Users must manually copy files. | **✅ Applicable & Critical (P0)**: Build hot SQLite backup (`VACUUM INTO` / `backup` API), cover archive bundling, manifest generation, integrity verification, and restore supervisor with WebUI & REST API. |
| **8** | **Strengthen security (Passwordless LAN-only)** | 🔒 Security | **Already 100% Implemented!** RFC 1918 + loopback subnet gating was completed in the previous session across WebUI, API, OPDS, Subsonic, and Jellyfin endpoints. | **✅ Already Complete (100%)** |
| **9** | **Add API / Device tokens (`DeviceToken`, Bearer auth)** | 🟠 High | Basic Auth and session cookies exist, but no revocable, scoped API/device tokens exist for clients like KOReader, smart TVs, or custom scripts. | **✅ Applicable & High Priority (P1)**: Implement `DeviceToken` model, `Authorization: Bearer ark_...` token authentication, token management API, and Settings UI panel. |
| **10** | **Introduce observability (Structured scan logs & health dashboard)** | 🟠 High | Basic `/api/health` exists, but no real-time status dashboard or structured scan stats exist in the WebUI. | **✅ Applicable & High Priority (P1)**: Add System Health panel in Settings displaying DB size, WAL status, FTS status, Watchdog status, FFmpeg availability, and scan metrics. |
