...
class OPDSProtocolPlugin(ProtocolPlugin):
    """OPDS 1.2 & 2.0 protocol plugin providing Atom XML and JSON-LD feeds."""

    name = "opds"
    display_name = "OPDS Catalog & Reading Sync"
    description = (
        "OPDS 1.2 (Atom XML), OPDS 2.0 (JSON-LD), and OPDS Progression 1.0 sync feeds"
    )
    protocol_version = "2.0"
    csrf_exempt = True
    blueprint_options: ClassVar[dict[str, Any]] = {"url_prefix": "/opds"}

    def register_routes(self, app: Flask | None = None) -> Blueprint | None:
        """Returns the OPDS protocol blueprint."""
        return opds_bp

    def check_health(self) -> dict[str, Any]:
        """Performs health check for the OPDS protocol plugin."""
        ...
        health.update(
            {
                ...
                "url_prefix": "/opds",
                "supported_types": sorted(OPDS_READABLE_TYPES),
            }
        )
        return health
