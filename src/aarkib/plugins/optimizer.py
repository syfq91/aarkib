"""E-Ink Device Hardware Optimizer plugin for on-demand EPUB transformations."""
...
from aarkib.plugins.base import OptimizerPlugin
...
class EInkOptimizerPlugin(OptimizerPlugin):
    """Hardware-tailored e-ink device optimizer plugin for EPUB media."""

    name = "eink_optimizer"
    display_name = "E-Ink Device Optimizer"
    description = "Hardware-specific EPUB optimizer with font-stripping, CSS sanitization, and 16-level grayscale dithering"
    supported_formats: ClassVar[set[str]] = {".epub"}
    ...
    def optimize(
        ...
        """Optimizes an EPUB file according to target device preset."""
        output_dir = cache_dir or Path(tempfile.gettempdir()) / "aarkib_optimized"
        clean_preset = preset.lower().strip() if preset else "generic"
        output_path = output_dir / f"{file_path.stem}_{clean_preset}.epub"
        return optimize_epub(file_path, output_path, preset_key=clean_preset)

    def check_health(self) -> dict[str, Any]:
        """Performs health check for the E-Ink optimizer plugin."""
