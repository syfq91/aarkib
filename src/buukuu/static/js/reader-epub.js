...
let currentFlow = localStorage.getItem("buukuu-reader-flow") || "paginated";
let currentTheme = localStorage.getItem("buukuu-reader-theme") || "dark";
let currentSpread = localStorage.getItem("buukuu-reader-spread") || "auto";
...
function adjustContentImages(contents) {
  ...
  const styleId = "buukuu-reader-injected-styles";
  ...
  if (images.length > 0) {
    ...
    if (isImageChapter && doc.body) {
      doc.body.classList.add("buukuu-fullpage-illustration");
...
function setReaderTheme(theme) {
  ...
  localStorage.setItem("buukuu-reader-theme", theme);
  if (rendition) {
...
function setColumns(cols) {
  currentSpread = cols;
  localStorage.setItem("buukuu-reader-spread", cols);
  updateSettingsUI();
...
function setFlow(flow) {
  currentFlow = flow;
  localStorage.setItem("buukuu-reader-flow", flow);
  updateSettingsUI();
